...
## 補足：プレイヤーの扱い
最初NPCとプレイヤーは別のクラスで扱おうとしていたが、最終的にはNPCクラス一つに統合した。

### 当初の計画
- Player と NPC ではゲームゲーム内の役割が異なる
- Player 特有のパラメータがあることを予想していた
- NPC にはスケジュールや AI専用のロジックが実装される可能性があった
...
- 特に多くの部分が同じで、別のクラスになるというのは避けたい

Player と NPC を統合してしまった方が実装が楽。

### UE と NPC クラスの連携は ?
ここで触れる話題ではないが、UE 側の Character とライブラリ内の NPC クラスの連携について軽く述べておく。
